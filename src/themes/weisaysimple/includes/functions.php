...
if (function_exists('register_sidebar'))
{
    register_sidebar(array(
		'name'			=> '小工具1',
        'before_widget'	=> '<div class="sidebar_t"><div class="sidebar_b">',
        'after_widget'	=> '',
        'before_title'	=> '<h3>',
        'after_title'	=> '</h3>',
    	'after_widget' => '</div></div><div class="sidebar_f"></div>',
    ));
}
{
    register_sidebar(array(
   ...
   <div id="div-comment-<?php comment_ID() ?>">
      <?php $add_below = 'div-comment'; ?>
		<div class="comment-author vcard">
			...
			<span class="reply"><?php comment_reply_link(array_merge( $args, array('reply_text' => '[回复]', 'add_below' =>$add_below, 'depth' => $depth, 'max_depth' => $args['max_depth']))); ?></span><strong><?php comment_author_link() ?></strong>：<span class="datetime">发表于 <?php comment_date('Y年m月d日') ?> <?php comment_time() ?> <?php edit_comment_link('编辑','&nbsp;&nbsp;',''); ?></span></div>
...
function colorCloudCallback($matches) {
    ...
    for($a=0;$a<6;$a++){    //采用#ffffff方法
       $color.=dechex(rand(4,15));//累加随机的数据--dechex()将十进制改为十六进制
    }
    $pattern = '/style=(\'|\")(.*)(\'|\")/i';
